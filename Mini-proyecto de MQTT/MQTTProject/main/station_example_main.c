                                ...
                                int32_t event_id, void* event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        ESP_LOGE(TAG, "Error de conexion. Codigo de motivo (reason code): %d", event->reason);
...
static void mqtt_app_start(void)
{
    ...
    ESP_LOGI(TAG, "Iniciando MQTT hacia el Host: %s", mqtt_cfg.broker.address.hostname);
    
    esp_mqtt_client_handle_t client = esp_mqtt_client_init(&mqtt_cfg);
    mqttClient = client;
    
    if (client == NULL) {
        ESP_LOGE(TAG, "Error: No se pudo inicializar el cliente (puntero nulo)");
        return;
    }
